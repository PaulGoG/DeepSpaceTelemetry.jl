...
using Dates: DateTime, Millisecond, now
...
function component_spawners(
    ...
    run_receiver_logged(attempt::Int) = with_logger(receiver_logger) do
        Receiver.run_receiver(
            ...
            max_batches_per_hour = telemetry.max_batches_per_hour,
...
batch-state timeline (`post_processing.generate_mask_timeline`) and the
point-wise 0/1 expansions (`expand_to_pointwise_masks`, rows from
`target_event_rows`). Each stage is failure-isolated: the simulation data
...
function post_process!(plan::MissionPlan, run_dir::String; orig_stdout::IO = stdout)
    ...
    get(pp, "expand_to_pointwise_masks", false) || return nothing
...
sequential pre-population of the onboard buffer, the mission clock with its
persisted anchor and absolute deadline, the supervised component tasks, and
the post-processing stages. Called by [`run_mission`](@ref), which owns the
lifecycle sentinels.
"""
