...
function get_batch_info(path)
    if !isdir(path) return Int[], Int[] end
    ...
    try
        ...
        while true
            ...
            if current_state_hash != last_state_hash
                ...
                target = isempty(active_ids) ? view_min : Float64(max(0, minimum(active_ids) - trail_buffer))
                ...
                p = scatterplot([view_lo, max_id], [y_lo, 3],
                                ...
                                ylabel="",
                                yticks=false,
                                width=plot_w, height=plot_h, color=:black)
                
                if !isempty(onb_l) scatterplot!(p, onb_l, fill(1, length(onb_l)), color=:red) end
                if !isempty(onb_a) scatterplot!(p, onb_a, fill(1, length(onb_a)), color=:magenta) end
                
                if !isempty(lnk_l) scatterplot!(p, lnk_l, fill(2, length(lnk_l)), color=:yellow) end
                if !isempty(lnk_a) scatterplot!(p, lnk_a, fill(2, length(lnk_a)), color=:blue) end 
                
                if !isempty(gnd_l) scatterplot!(p, gnd_l, fill(3, length(gnd_l)), color=:cyan) end
                if !isempty(gnd_a) scatterplot!(p, gnd_a, fill(3, length(gnd_a)), color=:green) end

                if !isempty(lost_ids) scatterplot!(p, lost_ids, fill(0, length(lost_ids)), color=:white) end

                show(IOContext(stdout, :color=>true), p)
                println()

                println("\n " * "-"^(border_w-2))
                print("  LIVE: "); printstyled("Satellite", color=:red); print(" → "); printstyled("In-Transit", color=:yellow); print(" → "); printstyled("Ground Archive", color=:cyan); println()
                print("  ARCH: "); printstyled("Satellite", color=:magenta); print(" → "); printstyled("In-Transit", color=:blue); print(" → "); printstyled("Ground Archive", color=:green); println()
                if !isempty(lost_ids)
                    print("  LOST: "); printstyled("$(length(lost_ids)) batch(es) dropped after retry exhaustion (bottom row)", color=:white); println()
                end
                println(" " * "-"^(border_w-2))
            end
